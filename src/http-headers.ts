import { parseRotation, Rotation, ZERO_ROTATION } from './rotation'
import { parseVector, Vector, ZERO_VECTOR } from './vector'

const LocalPosition: string = 'x-secondlife-local-position'
const LocalRotation: string = 'x-secondlife-local-rotation'
const LocalVelocity: string = 'x-secondlife-local-velocity'
const ObjectKey: string = 'x-secondlife-object-key'
const ObjectName: string = 'x-secondlife-object-name'
const OwnerKey: string = 'x-secondlife-owner-key'
const OwnerName: string = 'x-secondlife-owner-name'
const Region: string = 'x-secondlife-region'
const Shard: string = 'x-secondlife-shard'

// 
// This represents the headers received from the llHttpRequest() LSL function.
//
export class SecondLifeHttpHeaders {
...
export function parseSecondLifeHeaders(req: any, allowIncomplete = true): SecondLifeHttpHeaders | null {
    /*
    ...
    let position = parseVector(req.get(LocalPosition))
    let rotation = parseRotation(req.get(LocalRotation))
    let velocity = parseVector(req.get(LocalVelocity))

    if (!allowIncomplete && (position == null || rotation == null || velocity == null))
        return null

    if (!position) position = ZERO_VECTOR
    if (!rotation) rotation = ZERO_ROTATION
    if (!velocity) velocity = ZERO_VECTOR

    const result: SecondLifeHttpHeaders = {
